...
PARAMNUM=length(CROSSOVER)+length(NIND)+length(MAXGEN)+length(ELITIST)+length(PR_CROSS)...
    ...
    +length(InitializationMethode)+length(RepresentationMethode)+length(SurvivalMethode)+length(q);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Results Table
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
elements = {1:10,CROSSOVER ,NIND,MAXGEN,ELITIST,PR_CROSS,PR_MUT,stoppingCriteria...
    ,n_percentage,delta,InitializationMethode,RepresentationMethode,MutationMethode, SurvivalMethode,q}; %cell array with N vectors to combine
...
result.Properties.VariableNames ={'NumberOfRuns','CROSSOVER','NIND','MAXGEN'...
    ...
    ,'InitializationMethode','RepresentationMethode','MutationMethode', 'SurvivalMethode', 'q'};
...
for row = 1:size(result,1)
    if(mod(row-1,10)==0)
        ...
        [help_ShortestPath,help_Generations]=run_ga_Project2019(x, y, double(table2array(result(row,'NIND')))...
            ...
            ,table2array(result(row,'MutationMethode')),SelectionMethode,recombinMethode,double(table2array(result(row,'SurvivalMethode')))...
            ,double(table2array(result(row,'q'))));
        help_Time=toc;
...
writetable(result,'results_ParameterTuning_Task6_2.csv')
