...
% as described in the https://www.hindawi.com/journals/cin/2017/7430125/
% this crossover assumes that the path representation is used to represent
% TSP tours
%
% Syntax:  NewChrom = x_orderx(OldChrom, XOVR)
%
% Input parameters:
...
function NewChrom = x_orderx(OldChrom, XOVR);
...
[rows,cols]=size(OldChrom);
   ...
   for row=1:2:maxrows
	...
	if rand<XOVR			% recombine with a given probability
        [NewChrom(row,:), NewChrom(row+1,:)] = orderx([OldChrom(row,:);OldChrom(row+1,:)]);
	else
